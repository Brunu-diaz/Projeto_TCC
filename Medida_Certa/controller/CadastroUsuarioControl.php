...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($nome && $email && $username && $senha_provisoria) {
        ...
        // 7. Feedback para a View - Compatível com Alerta Flutuante
        if ($resultado) {
            // Redireciona com parâmetro de sucesso limpo
            header("Location: ../view/cadastrarusuario.php?sucesso=usuario_cadastrado");
            exit;
        } else {
            // Caso ocorra erro, passamos a mensagem via URL ou um código
            // urlencode garante que espaços e caracteres especiais não quebrem o link
            header("Location: ../view/cadastrarusuario.php?erro=" . urlencode("Erro técnico ao salvar no banco de dados."));
            exit;
        }
    } else {
